...
// ⛳ VK ID делает редирект сюда даже при PKCE, поэтому нужно вернуть страницу успеха
app.get('/auth/vk/callback', (req, res) => {
  res.sendFile(path.join(__dirname, 'public', 'success.html')); // 👈 может быть файлом-заглушкой или с JS
});

// ===========================
// 🔐 Приём токена от фронта после PKCE авторизации
// ===========================
// Этот маршрут вызывается из success.html (или другого файла)
// Он получает access_token, user_id и tg_id, и сохраняет их
app.post('/auth/vk/save', async (req, res) => {
  ...
  if (recentIPs.has(ip) && now - recentIPs.get(ip) < 3000) {
    console.warn(`⚠️ Повторный запрос с IP ${ip} — блок`);
